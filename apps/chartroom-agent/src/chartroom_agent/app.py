...
``AGENT_SURFACE`` picks which surface this process serves (ADR-56): chartroom
by default — the studio's design agent over chartroom-mcp — or registry, the
authoring surface's definition agent over registry-mcp. One surface per
process; the endpoint, the protocol and the loop are identical either way.

Startup opens a single supervised MCP session (the surface's server over
stdio) and builds the deep agent over an async SQLite checkpointer; a session
that dies is torn down and reopened on the next request rather than killing
the service.
...
(``document_id`` is the registry surface's synonym for ``dashboard_id`` —
the artifact the client had open.)
...
class Runtime:
    ...
    async def ensure(self) -> Any:
        ...
        client = make_client(self.surface, principal=os.environ.get("CHARTROOM_MCP_USER"))
        session = await self.stack.enter_async_context(client.session(self.surface.name))
        tools = assert_governed(await load_mcp_tools(session), self.surface)
        self.tool_names = sorted(t.name for t in tools)
        saver = await self.stack.enter_async_context(
            AsyncSqliteSaver.from_conn_string(
                os.environ.get("CHARTROOM_AGENT_DB", self.surface.checkpoint_db)
            )
        )
        self.agent = agent_mod.build_agent(tools, saver, self.surface.system_prompt)
        return self.agent
...
def _parse_turn(body: dict[str, Any]) -> tuple[str, str, str | None]:
    """(message, thread_id, context_id) from either accepted shape."""
    context_id = body.get("dashboard_id") or body.get("document_id") or None
    if isinstance(body.get("message"), str) and body["message"].strip():
        thread_id = str(body.get("thread_id") or uuid.uuid4())
        return body["message"], thread_id, context_id
    ...
    return message, str(uuid.uuid4()), context_id


def create_app(surface: Surface | None = None) -> FastAPI:
    surface = surface or surfaces.current()
    ...
    app = FastAPI(title=f"chartroom-agent ({surface.name})", lifespan=lifespan)
    app.state.runtime = runtime
    if surface.query_api:
    ...
    async def chat(request: Request) -> StreamingResponse:
        ...
        async def frames() -> AsyncIterator[str]:
            if not agent_mod.model_available():
                yield protocol.sse(protocol.unavailable(surface.unavailable_message))
                return
            try:
                message, thread_id, context_id = _parse_turn(body)
            ...
            try:
                ...
                return
            async for event in agent_mod.stream_turn(
                compiled, message, thread_id, context_id, surface.context_template
            ):
